-- 005_add_missing_columns_and_tables.sql
-- Add missing columns to existing tables first, then create new tables

-- Add missing columns to shows table
-- Check if columns exist before adding them
-- Note: SQLite doesn't have IF NOT EXISTS for columns, so we'll use a different approach

-- First, let's create the new tables that don't depend on column changes
...
-- Create indexes only on existing columns
...
-- Create indexes on existing performance columns
